...
public class StudentService {
    ...
    public AssignmentSubmission submitAssignmentByEmail(String email, SubmitAssignmentRequest request) {
        // Verify student is enrolled
        if (!enrollmentRepo.existsByStudentEmailAndCourseCode(email, request.getCourseCode())) {
            throw new StudentNotEnrolledInCourseException(email, request.getCourseCode());
        }

        AssignmentSubmission submission = AssignmentSubmission.builder()
                .studentId((long) email.hashCode())
                .assignmentNum(request.getAssignmentNum())
                .courseCode(request.getCourseCode())
                .assignmentTitle(request.getAssignmentTitle())
                .submissionContent(request.getSubmissionContent())
                .fileId(request.getFileUrl())
                .status("SUBMITTED")
                .build();
        return submissionRepo.save(submission);
    }
