...
public class StudentController {
    ...
    // The /profile endpoints using StudentProfile have been removed.

    @GetMapping("/grades")
    public ResponseEntity<?> getGrades(HttpServletRequest req) {
        Long userId = jwtExtractor.extractUserId(req);
        String auth = req.getHeader("Authorization");
        return ResponseEntity.ok(examServiceClient.getStudentGrades(auth, userId).getBody());
    }

    @GetMapping("/attendance")
    public ResponseEntity<?> getAttendance(HttpServletRequest req) {
        Long userId = jwtExtractor.extractUserId(req);
        String auth = req.getHeader("Authorization");
        return ResponseEntity.ok(attendanceServiceClient.getStudentAttendance(auth, userId));
    }
